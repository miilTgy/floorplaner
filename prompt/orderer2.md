你是一个资深 C++ 工程师。现在我已经有 Parser 模块输出的 `Problem` 数据结构（见 parser.h），请你实现 **ordering 模块**：`orderer2.cc`（以及必要时的 `orderer.h`），用于根据 PPT 的 cluster growth / linear ordering 思路生成一个 block 的初始排列 `perm`。
...
不再直接用 `net_count` 最大来选 seed，而是使用下面的综合评分：

\[
seed\_score(i)=\lambda \cdot normalized\_area(i) + (1-\lambda)\cdot normalized\_net\_count(i)
\]

其中 `0 <= lambda <= 1`，请把 `lambda` 写成模块内常量，初始建议：
...
实现时请使用 `long long` 计算面积，避免整型溢出。

##### (b) `net_count(i)`
块 i 的 net 数定义为：

\[
net\_count(i) = P.blocks[i].net_ids.size()
\]

由于 parser 已保证 `block.net_ids` 是 block 粒度且不重复，所以这里可以直接取长度。

##### (c) `normalized_area(i)`
对所有块的面积做 min-max 归一化：

\[
normalized\_area(i)=
\frac{area(i)-area_{min}}{area_{max}-area_{min}}
\]
...
特殊情况：
- 若 `area_max == area_min`，说明所有块面积都相同，则定义：
  - `normalized_area(i) = 1.0` 对所有 i 都成立

##### (d) `normalized_net_count(i)`
对所有块的 net_count 做 min-max 归一化：

\[
normalized\_net\_count(i)=
\frac{net\_count(i)-net_{min}}{net_{max}-net_{min}}
\]

其中：
- `net_min = 所有块 net_count 的最小值`
- `net_max = 所有块 net_count 的最大值`

特殊情况：
- 若 `net_max == net_min`，说明所有块的 net 数都相同，则定义：
  - `normalized_net_count(i) = 1.0` 对所有 i 都成立
...
- 遍历所有 block i，计算：
  ...
  - `net_count(i)`
  - `normalized_area(i)`
  - `normalized_net_count(i)`
  - `seed_score(i)`
- 选择 `seed_score` 最大的 block 作为 seed

tie-break（按顺序）：
1. `seed_score` 更大者优先
2. 若并列，`area(i)` 更大者优先
3. 若并列，`net_count(i)` 更大者优先
4. 若再并列，`block_id` 更小者优先
...
- 对每个 block i（按 i 从小到大）打印一行，包含：
  ...
  - `net_count`
  - `normalized_area`
  - `normalized_net_count`
  - `seed_score`
  - 是否为当前最佳(best)
- 示例格式（你可微调，但信息必须齐全且可读）：
  - `[ORDERER] seed-cand id=3 name=D area=279864 net_count=5 n_area=1 n_net=0.75 seed_score=0.925 best=YES`
- 最后一行打印最终 seed：
  - `[ORDERER] chosen-seed id=... name=... area=... net_count=... seed_score=...`
...
- `struct SeedCand { int id; std::string name; int net_count; long long area; double normalized_area; double normalized_net_count; double seed_score; bool chosen; };`
...
## 你需要输出什么
只输出 `orderer.h`（如有）和 `orderer2.cc` 的内容，不要解释，不要输出其他文件。